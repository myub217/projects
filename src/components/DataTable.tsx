          ...
          {data.length === 0 ? (
            ...
            data.map((row, idx) => (
              <tr
                key={idx}
                className={`${
                  onRowClick ? 'cursor-pointer hover:bg-base-200 dark:hover:bg-gray-800' : ''
                }`}
              ...
              >
                {columns.map(({ key, render, className: tdClass }) => (
                  <td key={String(key)} className={`px-3 py-2 align-top ${tdClass || ''}`}>
                    {render ? render(row[key], row) : String(row[key] ?? '')}
                  </td>
                ))}
                {actionColumn && (
                  <td className="px-3 py-2 text-center align-top">{actionColumn(row)}</td>
                )}
              </tr>
            ))
          )}
