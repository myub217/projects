...
const level = userLevel.toLowerCase(); const isMember = level === "member"; const isVip = level === "vip"; const isAdmin = level === "admin";
        ...
        <>
          <section className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-4">
            ...
            <div ref={businessRef} className="border rounded bg-white dark:bg-gray-900 flex justify-center overflow-x-auto">
              <iframe src="/business-registration.html" width="794" height="1123" className="shadow-md" title="ใบทะเบียนพาณิชย์" sandbox="allow-same-origin allow-scripts allow-forms" />
            </div>
            <div className="text-right space-x-2">
              ...
              {isAdmin && <button className="btn btn-sm btn-ghost" onClick={() => handlePrint(businessRef)}>พิมพ์เอกสาร</button>}
            </div>
          </section>
          ...
          {(isVip || isAdmin) && (
            <section className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-4">
              ...
              <div ref={salaryRef} className="border rounded bg-white dark:bg-gray-900 flex justify-center overflow-x-auto">
                <iframe src="/salary-certificate.html" width="794" height="1123" className="shadow-md" title="หนังสือรับรองเงินเดือน" sandbox="allow-same-origin allow-scripts allow-forms" />
              </div>
              <div className="text-right space-x-2">
                ...
                {isAdmin && <button className="btn btn-sm btn-ghost" onClick={() => handlePrint(salaryRef)}>พิมพ์เอกสาร</button>}
              </div>
            </section>
          )}
        </>
