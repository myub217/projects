import React from "react"; import "./SalaryCertificate.css";

interface SalaryDetail { label: string; amount: string; }

interface SalaryCertificateProps { companyThaiName: string; companyEngName: string; documentId: string; employeeName: string; startDate: string; position: string; department: string; salaryDetails: SalaryDetail[]; issueDate: string; issuerName: string; issuerPosition: string; hrPhone: string; hrAddressLines: string[]; watermarkUrl?: string; }

const SalaryCertificate: React.FC<SalaryCertificateProps> = ({ companyThaiName, companyEngName, documentId, employeeName, startDate, position, department, salaryDetails, issueDate, issuerName, issuerPosition, hrPhone, hrAddressLines, watermarkUrl = "sensor.webp", }) => { return ( <article className="page" role="document" lang="th"> {/* Watermark Layer */} <div style={{ position: "absolute", inset: 0, backgroundImage: url(${watermarkUrl}), backgroundPosition: "center center", backgroundRepeat: "no-repeat", backgroundSize: "90% auto", opacity: 0.2, zIndex: 0, pointerEvents: "none", }} />
