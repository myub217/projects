...
async function D(a, e) {
  ...
  const s = a.clone(),
    r = { headers: new Headers(s.headers), status: s.status, statusText: s.statusText },
  ...
  async fetch(e) {
    ...
    try {
      ...
      if (c instanceof Error)
        throw new l('plugin-error-request-will-fetch', { thrownErrorMessage: c.message })
    }
    const r = s.clone()
    try {
      let c
  ...
  async getCacheKey(e, t) {
    ...
    if (!this._cacheKeys[s]) {
      ...
      for (const r of this.iterateCallbacks('cacheKeyWillBeUsed'))
        n = m(await r({ mode: t, request: n, event: this.event, params: this.params }))
      this._cacheKeys[s] = n
    }
  ...
  *iterateCallbacks(e) {
  ...
  async _ensureResponseSafeToCache(e) {
    ...
    for (const n of this.iterateCallbacks('cacheWillUpdate'))
      if (
        ((t = (await n({ request: this.request, response: t, event: this.event })) || void 0),
        (s = !0),
        !t)
      )
        break
...
class K {
  ...
  async _awaitComplete(e, t, s, n) {
    ...
    try {
      ;(await t.runCallbacks('handlerDidRespond', { event: n, request: s, response: r }),
        await t.doneWaiting())
    } catch (i) {
      i instanceof Error && (c = i)
    }
    if (
      (await t.runCallbacks('handlerDidComplete', { event: n, request: s, response: r, error: c }),
      t.destroy(),
      c)
    )
      throw c
  ...
  async _handleFetch(e, t) {
    ...
    if (this._fallbackToNetwork) {
      const r = n.integrity,
        c = e.integrity,
        i = !c || c === r
      ;((s = await t.fetch(new Request(e, { integrity: e.mode !== 'no-cors' ? c || r : void 0 }))),
        r &&
          i &&
          e.mode !== 'no-cors' &&
  ...
  _useDefaultCacheabilityPluginIfNeeded() {
    ...
    for (const [s, n] of this.plugins.entries())
      n !== d.copyRedirectedCacheableResponsesPlugin &&
        (n === d.defaultPrecacheCacheabilityPlugin && (e = s), n.cacheWillUpdate && t++)
    t === 0
    ...
    return k(e, async () => {
      ...
      for (const [r, c] of this._urlsToCacheKeys) {
        const i = this._cacheKeysToIntegrities.get(c),
          ...
          h = new Request(r, { integrity: i, cache: o, credentials: 'same-origin' })
        await Promise.all(
          this.strategy.handleAll({ params: { cacheKey: c }, request: h, event: e })
        )
      }
...
class ne extends K {
  constructor(e = {}) {
    ;(super(e), this.plugins.some(t => 'cacheWillUpdate' in t) || this.plugins.unshift(ae))
  }
  async _handle(e, t) {
...
se(
  [
    ...
    { revision: 'a222dd5219009ff096813eefb483187a', url: 'manifest.webmanifest' },
  ] || []
)
